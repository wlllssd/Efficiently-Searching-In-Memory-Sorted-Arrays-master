...
class tip {
  ...
  Index interpolate(const Key y, const Index x_0, const Index x_1,
    ...
    double avgx=(x_0+x_1+x_2)/3.0,avgy=(data[x_0]+data[x_1]+data[x_2])/3.0;
    double k=((x_0-avgx)*(data[x_0]-avgy)+(x_1-avgx)*(data[x_1]-avgy)+(x_2-avgx)*(data[x_2]-avgy))/((x_0-avgx)*(x_0-avgx)+(x_1-avgx)*(x_1-avgx)+(x_2-avgx)*(x_2-avgx));
