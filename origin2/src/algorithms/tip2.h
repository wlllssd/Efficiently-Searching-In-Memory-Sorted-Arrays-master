...
class tip2 {
  ...
  Index interpolate(const Key y, const Index x_0, const Index x_1,
                    const Index x_2,const Index left4,const Index right4) const {
   /*                   
    double y_0 = data[x_0] - y, y_1 = data[x_1] - y, y_2 = data[x_2] - y,
        ...
        (y_2 * (x_1 - x_2) * (y_0 - y_1) +
            y_0 * (x_1 - x_0) * (y_1 - y_2));
    */
    double expect1 = (y-data[x_1])*(y-data[x_2])*x_0/(data[x_0]-data[x_1])/(data[x_0]-data[x_2]);
    double expect2 = (y-data[x_0])*(y-data[x_2])*x_1/(data[x_1]-data[x_0])/(data[x_1]-data[x_2]);
    double expect3 = (y-data[x_1])*(y-data[x_0])*x_2/(data[x_2]-data[x_1])/(data[x_2]-data[x_0]);
    return (Index)(expect1+expect2+expect3);
 ...
 public:
  ...
  __attribute__((always_inline)) Key search(const Key x) {
    Index left = 0, right = data.size() - 1, next_1 = data.size()*2/4,left4=data.size()/4,right4=data.size()*3/4,
        next_2 = interpolate(x);
    while(true) {
      if (next_2 - left4 <= guard_off && next_2 - left4 >= -guard_off)
        return linear_search(x, next_2);
      if (next_2 - right4 <= guard_off && next_2 - right4 >= -guard_off)
        return linear_search(x, next_2);
    if (next_2 - next_1 <= guard_off && next_2 - next_1 >= -guard_off)
      ...
      next_2 = interpolate(x, left, next_1, right,left4,right4);
